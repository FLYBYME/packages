...
export class RaftNode extends EventEmitter implements IRaftNode {
    ...
    async start(): Promise<void> {
        ...
        const stateRow = await this.storage.get('SELECT current_term, voted_for, last_applied FROM raft_state WHERE id = 1');
        if (stateRow) {
            const row = stateRow as { current_term: number, voted_for: string | null, last_applied?: number };
            this.currentTerm = row.current_term;
            this.votedFor = row.voted_for;
            this.lastApplied = row.last_applied || 0;
        } else {
             await this.storage.run('CREATE TABLE IF NOT EXISTS raft_state (id INTEGER PRIMARY KEY, current_term INTEGER, voted_for TEXT, last_applied INTEGER)');
             await this.storage.run('INSERT OR IGNORE INTO raft_state (id, current_term, voted_for, last_applied) VALUES (1, 0, NULL, 0)');
        }
    ...
    public async persistState(): Promise<void> {
        await this.storage.run('UPDATE raft_state SET current_term = ?, voted_for = ?, last_applied = ? WHERE id = 1', [this.currentTerm, this.votedFor, this.lastApplied]);
    }
    ...
    private registerHandlers(): void {
        ...
        this.network.on('append-res', async (msg: RaftMessage) => {
             await this.replicationManager.handleAppendResponse(msg.data as AppendEntriesReply, msg.senderNodeID);
        });
        ...
        this.network.on('snapshot-res', async (msg: RaftMessage) => {
            await this.replicationManager.handleInstallSnapshotResponse(msg.data as InstallSnapshotReply, msg.senderNodeID);
        });
    }

    public async stepDown(term: number): Promise<void> {
    ...
    public async applyCommitted(): Promise<void> {
        ...
        while (this.lastApplied < this.commitIndex) {
            ...
            if (entry) {
                ...
                if (ledger) {
                    const tx = {
                        txID: entry.id || entry.txID || `temp-${entry.index}-${entry.term}`,
                        prevTxID: entry.prevID || entry.prevTxID || null,
                        index: entry.index,
                        term: entry.term,
                    ...
                    await ledger.appendLocally(tx as unknown as import('../dlt/dlt.types').Transaction<unknown>).catch((err: unknown) => {
                         const errorMessage = err instanceof Error ? err.message : 'Unknown apply error';
    ...
    public getOrCreateLedger<T = unknown>(namespace: string): DistributedLedger<T> {
        if (!this.ledgers.has(namespace)) {
            ...
            this.ledgers.set(namespace, ledger as unknown as DistributedLedger<unknown>);
        }
