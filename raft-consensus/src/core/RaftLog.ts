...
export class RaftLog {
    ...
    public async getEntry(index: number): Promise<LogEntry | null> {
        ...
        if (typeof payload === 'string') {
            try { payload = JSON.parse(payload); } catch { /* ignore parse error */ }
        }
        return { ...row, payload } as unknown as LogEntry;
    }

    public async getEntriesFrom(startIndex: number, limit?: number): Promise<LogEntry[]> {
        let query = 'SELECT * FROM raft_log WHERE [index] >= ? ORDER BY [index] ASC';
        const params: unknown[] = [startIndex];
        ...
        const rows = await this.provider.all(query, params);
    ...
    public async getLastLogIndex(): Promise<number> {
        ...
        if (lastIndex === 0) {
            ...
            return snapshot ? (snapshot as { last_index: number }).last_index : 0;
        }
        return lastIndex;
    }
    ...
    public async getLastLogTerm(): Promise<number> {
        ...
        const snapshot = await this.getLatestSnapshot();
        return snapshot ? (snapshot as { last_term: number }).last_term : 0;
    }
    ...
    public async getTerm(index: number): Promise<number> {
        ...
        const snapshot = await this.getLatestSnapshot();
        if (snapshot && index === (snapshot as { last_index: number }).last_index) return (snapshot as { last_term: number }).last_term;
     ...
     */
    public async createSnapshot(lastIndex: number, lastTerm: number, data: unknown): Promise<void> {
        await this.provider.run(
    ...
    public async getLatestSnapshot(): Promise<{ last_index: number, last_term: number, data: unknown } | null> {
